...
void O3_CPU::read_from_trace()
{
    ...
    // How many blocks with a branch instruction have been fetched and predicted this cycle
    uint8_t blocks_predicted = 0;
    uint8_t continue_fetching_blocks = 1;

    // Store the prediction and actual result for each block fetched in the cycle
    // Index 0 is from the first block, index 1 is from the second block, etc
    uint8_t last_branch_predictions[LEVELS_TO_PREDICT];
    uint8_t last_branch_results[LEVELS_TO_PREDICT];
    FILE snapshot_file[LEVELS_TO_PREDICT];

    // Fetch multiple blocks loop
    // This will break when LEVELS_TO_PREDICT blocks have been read OR an entire block has been read without a branch
    while(continue_fetching_blocks)
    {
        uint8_t continue_reading = 1;
        uint32_t num_reads = 0;
        instrs_to_read_this_cycle = FETCH_WIDTH;

        // Save the levels of prediction
        ...
        while (continue_reading)
        {
            ...
            { // successfully read the trace
                ...
                {
                    ifetch_buffer_index = add_to_ifetch_buffer(&arch_instr);
                    num_reads++;

                    // handle branch prediction
                    if (IFETCH_BUFFER.entry[ifetch_buffer_index].is_branch)
                    {
                        // We have encountered a branch! Snapshot the current file pointer in case of misprediction (saving FROM pointer value NOT pointer value)
                        snapshot_file[blocks_predicted] = *trace_file;
                        ...
                        // TODO: Pass in 'last_branch_predictions' array to the branch predictor to make predictions based on multiple past predictions
                        // Also pass in 'blocks_predicted' which is the size of the array
                        // uint8_t branch_prediction = predict_branch(IFETCH_BUFFER.entry[ifetch_buffer_index].ip); previous implementation
                        uint8_t branch_prediction = predict_branch(last_branch_predictions, blocks_predicted, IFETCH_BUFFER.entry[ifetch_buffer_index].ip);

                        // Store all predictions from the current cycle here
                        last_branch_predictions[blocks_predicted] = branch_prediction;

                        // Store actual branch results here to update the global history register later
                        last_branch_results[blocks_predicted] = IFETCH_BUFFER.entry[ifetch_buffer_index].branch_taken;


                        // This block of code sets up the 'predicted_branch_target' variable which is only ever used once
                        // The one usage is in the 'l1i_prefetcher_branch_operate' function below, but the function is empty,
                        // so it's probably safe to omit all of this
                        ...
                        if (IFETCH_BUFFER.entry[ifetch_buffer_index].branch_taken != branch_prediction)
                        {
                            ...
                            {
                                // Prediction was incorrect so add cycle penalty
                                fetch_stall = 1;
                                IFETCH_BUFFER.entry[ifetch_buffer_index].branch_mispredicted = 1;
                            }
                        }

                        // Done fetching this block
                        continue_reading = 0;

                        blocks_predicted++;
                        if(blocks_predicted >= LEVELS_TO_PREDICT)
                        {
                            // Maximum blocks fetched
                            continue_fetching_blocks = 0;
                        }
                    }
                    else
                    {
                        // The instruction was not a branch instruction
                        // Check if we need to continue reading instruction in the block
                        if (num_reads >= instrs_to_read_this_cycle)
                        {
                            // End of the block was reached with no branches
                            // Done reading and fetching blocks
                            continue_reading = 0;
                            continue_fetching_blocks = 0;
                        }
                    }

                    // Finish fetching if buffer is full
                    if(IFETCH_BUFFER.occupancy == IFETCH_BUFFER.SIZE)
                    {
                        continue_reading = 0;
                        continue_fetching_blocks = 0;
        ...
        } // Fetch instruction in block while loop
    } // Fetch block while loop

    // Check if branches were predicted this cycle
    if(blocks_predicted > 0)
    {
        // RESOLVED TODO: Call last_branch_result and pass in last_branch_results so the global history register can be updated if there were incorrect predictions
        last_branch_result(last_branch_results, blocks_predicted, IFETCH_BUFFER.entry[ifetch_buffer_index].ip);

        // RESOLVED TODO: If there were mispredictions, move file pointer back to where the first branch was mispredicted 
        for (int i = 0 ; i < blocks_predicted ; i++)
        {
            if (last_branch_predictions[i] != last_branch_results[i])
            {
                *trace_file = snapshot_file[i];
                num_branch - (blocks_predicted - 1 - i);
...
void O3_CPU::complete_execution(uint32_t rob_index)
{
    if (ROB.entry[rob_index].is_memory == 0)
    {
        if ((ROB.entry[rob_index].executed == INFLIGHT) && (ROB.entry[rob_index].event_cycle <= current_core_cycle[cpu]))
        {
            ...
            {
                fetch_resume_cycle = current_core_cycle[cpu] + BRANCH_MISPREDICT_PENALTY*fetch_stall;
            }
