...
class O3_CPU {
  public:
    ...
    CORE_BUFFER IFETCH_BUFFER{"IFETCH_BUFFER", FETCH_WIDTH*LEVELS_TO_PREDICT*2};
    CORE_BUFFER DECODE_BUFFER{"DECODE_BUFFER", DECODE_WIDTH*LEVELS_TO_PREDICT*3};
  ...
  // trace cache for previously decoded instructions
    ...
    // branch predictor
    // uint8_t predict_branch(uint64_t[] ip); Previous implementation
    uint8_t predict_branch(uint8_t previous_branch_predictions[], uint8_t branch_predicted, uint64_t fetch_address);
    void    initialize_branch_predictor();
    void   last_branch_result(uint8_t actual_branches_taken[], uint8_t branches_found, uint64_t fetch_address);
